...
def brut2(cores='all'):   
    ...
    workers = []
    for r in range(cores):
        p = Process(target=main2, args=(counter, r))
        workers.append(p)
        p.start()
    ...
    while True:
        ...
        if k % screen_print_after_keys == 0:
            ...
            if addr in add:
                ...
                f=open('winner.txt','a')
                f.write('\n FOUND!!!' + '\nPrivateKey= ' + pvk.hex() + '\nAddress =' + addr, '\nMinikey = ' + MINIKEY)
                f.write('\n===== Made by EvgenyUnholy |  Donations 35dex592mVX6EiF6ag9ZJzWAQrobz3ciDu =====')
                f.close()
                found += 1
                return
            else:
...
def main2(counter, r):
    ...
    while True:
        ...
        if k % screen_print_after_keys == 0:
            ...
            print('Chck: {} Fnd: {} Keys/s: {} Addr: {} Mini: {}'.format(counter.value, found, round(counter.value/(time.time() - st)), addr, MINIKEY), end='\r')
            if addr in add:
                print('\n FOUND!!!', '\nPrivateKey= ', pvk.hex(), '\nAddress = ', addr, '\nMinikey = ', MINIKEY)
                f=open('winner.txt','a')
                f.write('\n FOUND!!!' + '\nPrivateKey= ' + pvk.hex() + '\nAddress =' + addr, '\nMinikey = ' + MINIKEY)
    ...
    while True:
        ...
        for i in itertools.product('123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz', repeat=21):
            ...
            if k % screen_print_after_keys == 0:
                ...
                print('Checked: {} Found: {} Keys/s: {} Minikey: {} Addr: {}'.format(k, found, round(k/(time.time() - st)), MINIKEY, addr), end='\r')
                if addr in add:
                    print('\n FOUND!!!', '\nPrivateKey= ', pvk.hex(), '\nAddress = ', addr, '\nMinikey = ', MINIKEY)
                    f=open('winner.txt','a')
                    f.write('\n FOUND!!!' + '\nPrivateKey= ' + pvk.hex() + '\nAddress =' + addr, '\nMinikey = ' + MINIKEY)
...
if __name__ == '__main__':
    symb = int(input("===== Made by EvgenyUnholy |  Donations 35dex592mVX6EiF6ag9ZJzWAQrobz3ciDu =====  https://github.com/evgenyunholy \n\n1 - 22 symbols, print all keys; \n2 - 22 symbols, print only valid keys; \n3 - 30 symbols, print all keys; \n4 - 30 symbols, print only valid keys; \n5 - 22 symbols, print all keys, FULL BRUT \nChoose format and display print: "))
    if symb == 1:
        ...
        brut2(cores = int(input("Cores count: ")))
        pass  
    elif symb == 4:
        brut4(cores = int(input("Cores count: ")))
        pass    
    elif symb == 5:
        brut5(cores = int(input("Cores count: ")))
        pass              
    else:
